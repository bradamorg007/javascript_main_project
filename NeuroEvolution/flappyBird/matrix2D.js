...
class Matrix2D {
    ...
    static divide(a, b) {
        ...
        if (aIsScalar && !bIsScalar) {
            ...
            if (a === 0) {
                throw new Error("Illegal Argument Exception: Can not divide by zero");
            }
            return new Matrix2D(b.size().rows, b.size().cols)
                .map((_, i, j) => b.data[i][j] / a);
            ...
            throw new Error("Illegal Argument Exception: Divide method only performs scalr division to avoid element wise matrix divisions by 0")
        }
    }
    ...
    static map(matrix, func) {
        ...
        return new Matrix2D(matrix.size().rows, matrix.size().cols)
